// =============================================
//  MOBILE SIZING — runs before everything
//  Sets --ctrl-h and --canvas-h CSS vars so
//  both the controller and canvas size correctly
//  the moment the game panel becomes visible.
// =============================================
(function mobileSizing() {
    function setControllerHeight() {
        ...
        // Ideal controller height = vw / 1.988, clamped 180-302
        const ctrlH = Math.min(302, Math.max(180, vw / 1.988));
        // Use dvh when available so the URL bar doesn't cause gaps
        ...
        // Also size the canvas element directly so the browser
        // actually applies the height before the first repaintAll()
...
function repaintAll() {
    ...
    if (images.heli?.complete    && images.heli.naturalWidth    > 0) g2d.drawImage(images.heli,    200, 100, 200, 100);
    ...
    // ---- SYSTEM MESSAGE using ip.png as board ----
    if (!isAnimating) {
        ...
        if (systemMessage) {
            drawInfoBoard(g2d, systemMessage);
        }
    }

    // Dice image (bottom center)
    if (currentDiceImg?.complete && currentDiceImg.naturalWidth > 0) {
        g2d.drawImage(currentDiceImg, 250, 900, 100, 100);
    }
...
function drawSign(g2d, x, y, l1, l2, pc) {
    if (images.scoreBgImg?.complete && images.scoreBgImg.naturalWidth > 0) {
        ...
        g2d.fillStyle = "#8B4513"; g2d.fillRect(x, y, 180, 80);
    }
    g2d.fillStyle = "#FFF"; g2d.font = "bold 20px Courier New";
    g2d.fillText(l1, x + 20, y + 35);
    g2d.font = "13px Arial"; g2d.fillStyle = pc;
...
function drawTargetSign(g2d, x, y, text) {
    if (images.scoreBgImg?.complete && images.scoreBgImg.naturalWidth > 0) {
        ...
        g2d.fillStyle = "#8B4513"; g2d.fillRect(x, y, 120, 50);
    }
...
function drawRoundSign(g2d, x, y, text) {
    ...
    if (images.scoreBgImg?.complete && images.scoreBgImg.naturalWidth > 0) {
        ...
        g2d.fillStyle = "#654321"; g2d.fillRect(x, y, 120, 30);
    }
    ...
    try {
        ...
        if ((n + 1) % 3 === 0) {
            ...
            g2d.fillText(wt, x + 60 - g2d.measureText(wt).width / 2, y + 50);
        }
    } catch (e) {}
}
...
function drawGameOverScreen(g2d) {
    ...
    if (pi?.complete && pi.naturalWidth > 0) {
        ...
        g2d.fillStyle = "#FFF"; g2d.fillText(gameOverWinner, px + pw / 2, py + 100);
        g2d.restore();
    }
    g2d.save(); g2d.textAlign = "left";
    g2d.shadowColor = "rgba(0,0,0,0.95)"; g2d.shadowBlur = 6;
    g2d.shadowOffsetX = 2; g2d.shadowOffsetY = 2;
...
function finalizeRoll() {
    if (isDecidingTurn) {
        ...
        if (rollingForPlayer === 1) {
            ...
            if (or > 1 && rv === 1) {
                p2StartRoll = -1;
                systemMessage = "🎲 P1 rolled 1 in Round " + or + "! Moving first.";
            } else if (p2StartRoll === 0) rollingForPlayer = 2;
        } else {
            p2StartRoll = rv;
            if (or > 1 && rv === 1) {
                p1StartRoll = -1;
                systemMessage = "🎲 P2 rolled 1 in Round " + or + "! Moving first.";
        ...
        if (isTeleportPending) executePendingTeleport();
            ...
            if (currentTurn === 1) p1.startTimer();
            else p2.startTimer();
        }
    }
    isAnimating = false;
